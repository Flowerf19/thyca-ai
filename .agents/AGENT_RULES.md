...
- ActiveMemory is `thyca/memory/active.py` only: `SOUL`/`USER` full inject; `MEMORY` + daily tail `hotTailKB`. Archived/L2 is `thyca/memory/archived.py` + `chunk.py` — closed daily + always-on canonical index, not injected. Facade/`memory_*` is Tools, not ActiveMemory.
