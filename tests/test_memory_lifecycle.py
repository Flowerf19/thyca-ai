...
def test_expired_deleted_on_reindex(tmp_path: Path) -> None:
    t0 = datetime(2026, 8, 1, 12, 0, tzinfo=timezone.utc)
    facade = MemoryFacade(tmp_path, timezone_name="Asia/Ho_Chi_Minh")
    facade.remember("x", "short lived", target="memory", importance=1, now=t0)
    assert "short lived" in (tmp_path / "MEMORY.md").read_text(encoding="utf-8")
    assert TTL_DAYS[5] == 180
    facade._refresh_index(t0 + timedelta(days=4))
    assert "short lived" not in (tmp_path / "MEMORY.md").read_text(encoding="utf-8")
    assert facade.search("short lived", now=t0 + timedelta(days=4)).hits == []


def test_reject_forget_soul(tmp_path: Path) -> None:
    facade = MemoryFacade(tmp_path, timezone_name="Asia/Ho_Chi_Minh")
    facade.remember("me", "I am me", target="soul")
    try:
